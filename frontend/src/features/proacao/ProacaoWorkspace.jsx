/**
 * PRÓ-AÇÃO — Propostas de melhoria + TPM (reutilizável na página /app/proacao e embutido no Chat Impetus).
 */
...
/**
 * @param {{ variant?: 'page' | 'embedded', initialSection?: 'proposals' | 'tpm' }} props
 */
export default function ProacaoWorkspace({ variant = 'page', initialSection = 'proposals' }) {
  const isEmbedded = variant === 'embedded';
  const notify = useNotification();
  const [searchParams] = useSearchParams();
  const [section, setSection] = useState(() =>
    initialSection === 'tpm' ? 'tpm' : !isEmbedded && searchParams.get('tab') === 'tpm' ? 'tpm' : 'proposals'
  );

  /* Embedded: aplicar aba antes da pintura para não flashar em “Propostas”. */
  useLayoutEffect(() => {
    if (!isEmbedded) return;
    setSection(initialSection === 'tpm' ? 'tpm' : 'proposals');
  }, [isEmbedded, initialSection]);
  ...
  return (
    <div className={`content proacoes-content ${isEmbedded ? 'proacao-workspace--embedded' : ''}`}>
