...
/**
 * AIOI-P8.2 — SSR helper Runtime Authorization (test-only)
 */

const path = require('path');
const { createRequire } = require('module');

const AUTHORIZATION_ROOT = path.resolve(__dirname, '..');
const TESTS_ROOT = __dirname;

async function bundleRuntimeAuthorizationProviderSsr() {
  const esbuild = require('esbuild');
  const entry = path.join(TESTS_ROOT, 'ExecutiveRuntimeAuthorizationSsrEntry.jsx');
  ...
  const mod = requireBundledModule(AUTHORIZATION_ROOT, 'runtime-authorization', result.outputFiles[0].text);
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                React.createElement(
                  ...
                  null,
                  React.createElement('div', { 'data-testid': 'auth-child' }, 'Child')
                )
              )
            )
          )
...
module.exports = { bundleRuntimeAuthorizationProviderSsr };
