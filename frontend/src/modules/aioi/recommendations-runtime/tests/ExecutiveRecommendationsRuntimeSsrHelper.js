...
/**
 * AIOI-P8.5 — SSR helper Recommendations Runtime (test-only)
 */

const path = require('path');
const { createRequire } = require('module');

const RECOMMENDATIONS_RUNTIME_ROOT = path.resolve(__dirname, '..');
const TESTS_ROOT = __dirname;

async function bundleRecommendationsRuntimeProviderSsr() {
  const esbuild = require('esbuild');
  const entry = path.join(TESTS_ROOT, 'ExecutiveRecommendationsRuntimeSsrEntry.jsx');
                  ...
                  React.createElement(
                    ...
                    React.createElement(
                      ...
                      React.createElement(
                        ...
                        null,
                        React.createElement('div', { 'data-testid': 'recommendations-runtime-child' }, 'Child')
                      )
                    )
                  )
                )
...
module.exports = { bundleRecommendationsRuntimeProviderSsr };
