...
export function useChatSocket({ onMessage, onTyping, onStopTyping, onOnline, onOffline, onReaction, onRead }) {
  const socketRef = useRef(null);
  const h = useRef({});
  useEffect(() => { h.current = { onMessage, onTyping, onStopTyping, onOnline, onOffline, onReaction, onRead }; });
  useEffect(() => {
    ...
    return () => { ['new_message','user_typing','user_stop_typing','user_online','user_offline','message_reaction','messages_read'].forEach(e => s.off(e)); };
  }, []);
