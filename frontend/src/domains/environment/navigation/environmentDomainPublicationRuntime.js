...
function publicationUrl() {
  return `${API_URL.replace(/\/+$/, '')}/environment-navigation/context`;
}
...
export async function fetchEnvironmentPublicationContext() {
  const now = Date.now();
...
export function invalidateEnvironmentPublicationCache() {
  _cached = null;
  _cachedAt = 0;
}
