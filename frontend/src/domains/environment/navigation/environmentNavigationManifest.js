/**
 * Manifesto ENVIRONMENT — SGA/EHS enterprise (rotas canónicas /app/environment/operational).
 */

/** @type {Array<{ id: string, label: string, path: string, requires: object, bands: string[] }>} */
export const ENVIRONMENT_NAVIGATION_MANIFEST = Object.freeze([
  {
  ...
  {
    id: 'environment_cognitive',
    label: 'Cognitivo ambiental',
    path: '/app/environment/operational?view=cognitive',
    requires: { cognitive: true },
    bands: ['analyst', 'coordinator', 'manager', 'director']
  },
  {
    id: 'environment_executive',
    label: 'Cockpit executivo',
    path: '/app/environment/operational?view=executive',
...
export function listEnvironmentManifestRoutesForTests() {
  return ENVIRONMENT_NAVIGATION_MANIFEST.map((m) => m.path);
}
