...
export function markEnvironmentRouteOpen(pathname) {
  _routeOpenAt.set(String(pathname || '/'), Date.now());
}

export async function recordEnvironmentBehaviorEvent(evt) {
  try {
    const token = localStorage.getItem('impetus_token');
    if (!token) return { ok: false };
    const { API_URL } = await import('../../../services/api.js');
    const res = await fetch(`${API_URL.replace(/\/+$/, '')}/environment-operational-validation/behavior/event`, {
      method: 'POST',
