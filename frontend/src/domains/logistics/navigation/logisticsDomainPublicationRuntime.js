...
function publicationUrl() {
  return `${API_URL.replace(/\/+$/, '')}/logistics-navigation/context`;
}
...
export async function fetchLogisticsPublicationContext() {
  const now = Date.now();
...
export function invalidateLogisticsPublicationCache() {
  _cached = null;
  _cachedAt = 0;
}
