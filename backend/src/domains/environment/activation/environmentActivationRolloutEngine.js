...
function resolveActivationStage() {
  const s = String(process.env.IMPETUS_ENVIRONMENT_ACTIVATION_STAGE || 'shadow').toLowerCase();
...
function allowsDefinitivePublication(stage, shadowModeEnv) {
  const shadow =
    String(shadowModeEnv || process.env.IMPETUS_ENVIRONMENT_PUBLICATION_SHADOW_MODE || '').toLowerCase() ===
    'true';
