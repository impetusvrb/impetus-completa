...
/**
 * EVENT-GOVERNANCE-11C — adapter ESG → Event Governance.
 * Orquestra distribuição com escalonamento corporativo (níveis 1–4, modelo SST).
 * Indicadores, inventários, medições e compliance permanecem inalterados.
 */
...
} = require('../esgNotificationService');

const METRIC_EVENTS = 'event_governance_esg_events';
const METRIC_MIGRATED = 'event_governance_esg_migrated';
const METRIC_SHADOW_TOTAL = 'event_governance_esg_shadow_total';
const METRIC_SHADOW_MATCH = 'event_governance_esg_shadow_match';
const METRIC_SHADOW_DIVERGENCE = 'event_governance_esg_shadow_divergence';

const POLICY_ID = 'ESG_LIFECYCLE';
...
function isEsgGovernanceEnabled() {
  return String(process.env.EVENT_GOVERNANCE_ESG || '').toLowerCase() === 'true';
}
...
function buildGovernanceEvent(input) {
  const eventType = input.eventType || 'esg_generic';
...
function inferLegacyDistribution(input) {
  ...
  return {
    ...
    escalationLabel: ESG_ESCALATION_LEVELS[escalationLevel]?.label || null
  };
}
...
/**
 * Fluxo legado — NC + escalonamento ESG.
 * @param {object} input
 */
async function runLegacyDistribution(input) {
  const esg = require('../esgNotificationService');
  return esg.executeLegacyDistribution(input);
}
 ...
 * @param {object} input
 */
async function dispatchEsgNotification(input) {
  if (!input.companyId) {
  ...
  const migrated = isEsgGovernanceEnabled();
  ...
  try {
    ...
    console.warn('[esgGovernanceAdapter][dispatch]', err?.message ?? err);
    return {
...
function getAuditStatus() {
  ...
  const enabled = isEsgGovernanceEnabled();
  ...
  return {
    ...
    escalation_levels: Object.keys(ESG_ESCALATION_LEVELS).map(Number)
  };
}
...
module.exports = {
  isEsgGovernanceEnabled,
  buildGovernanceEvent,
  inferLegacyDistribution,
  compareShadow,
  dispatchEsgNotification,
