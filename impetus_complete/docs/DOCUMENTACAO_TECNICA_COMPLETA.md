...
1. **Comunicação Rastreada Inteligente** — Captura, classificação e roteamento automático de mensagens via App Impetus
...
|----------------------|---------------------------|
| **Captura automática de comunicações** | App Impetus (`POST /api/app-impetus/messages`), gravação em `communications` |
...
| **Resposta automática** | `appImpetusService.sendMessage()` após classificação |
...
| **Formulário TPM** | Sessões conversacionais via App Impetus, persistência em `tpm_incidents`, `tpm_shift_totals` |

### 2.2 Fluxo de Comunicação (Processo Registrado)

```
[Operador/Colaborador] → App Impetus
    ↓
[App] POST /api/app-impetus/messages
    ↓
[Backend] processIncomingFromApp() → salva em communications
    ↓
[IA] tpmConversation.processMessage() — se ativo: fluxo TPM
    OU ai.processIncomingMessage() — classificação + tarefa/diagnóstico
    ↓
[Resposta] appImpetusService.sendMessage() — confirmação ao remetente
```
...
│   │   │   ├── app_impetus.js  # Canal App Impetus
│   │   │   ├── manuals.js
│   │   │   ├── tasks.js
│   │   │   ├── proacao.js
│   │   │   ├── alerts.js
...
│   │   │   ├── appImpetusService.js # Canal App Impetus
│   │   │   ├── proacao.js
...
| Mensagens | App Impetus (canal integrado) |
...
| POST | `/api/app-impetus/messages` | Canal App Impetus — grava em `communications`, processa IA e TPM |
...
| `communications` | Mensagens rastreadas (App Impetus, web) |
...
| `app_impetus_outbox` | Mensagens pendentes; tabelas `zapi_*` legado |
...
2. **Se falha_técnica:**
   ...
   - Resposta App: "Diagnóstico gerado e tarefa criada" + oferta formulário TPM
3. **Se tarefa:**
   - Cria tarefa com descrição = texto
   - Resposta App: "Tarefa registrada com sucesso"
...
Fluxo conversacional via App Impetus:
...
4. Ao confirmar: grava em `tpm_incidents`, atualiza `tpm_shift_totals`, notifica gestores via App Impetus e cria alerta em `alerts`
...
| (ZAPI_* removido) | Canal atual: App Impetus via `app_impetus_outbox` |
| `SALT` | Salt para tokens/licença |
| `ENCRYPTION_KEY` | Criptografia de secrets (legado; tokens Z-API não mais usados) |
...
- **Mensagens do App não chegam:** verificar conectividade do App com o backend, rotas `/api/app-impetus/*`
