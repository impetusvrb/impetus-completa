...
router.get('/data-access', 
  ...
  async (req, res) => {
    try {
      ...
      // Ambiente legado: se a tabela LGPD ainda não existir, não quebrar a tela.
      if (err?.code === '42P01') {
        return res.json({
          ok: true,
          logs: [],
          pagination: {
            total: 0,
            limit: safeInteger(req.query.limit, 100, 1, 500),
            offset: safeInteger(req.query.offset, 0, 0, 100000)
          }
        });
      }
      res.status(500).json({
